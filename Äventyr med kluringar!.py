...
def speedGame(situation):
    ...
    while True:
        t1 = time.time() 									#Checks the current time
        playerAnswer = checkAnswer(input("Answer here: "))  
        t2 = time.time()									#Checks the current time
        t = t2 - t1											#Compares and get a difference
...
def checkAnswer(playerInput):  
    step1 = playerInput.replace(" ", "") #Removes spaces
    step2 = step1.lower() 				 #Changes symbols to lower case
    return(step2)
